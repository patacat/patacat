...
var Cat = /** @class */ (function () {
    ...
    Cat.prototype.update = function (time) {
        ...
        if (this.patted) {
            if (this.pattedTime === 0) {
                ...
                Cat.currentCats = Cat.currentCats.filter(function (c) { return c !== _this; });
                this._deleted = true;
    ...
    Cat.prototype.draw = function (ctx, gs) {
        if (this._deleted)
            return;
        ...
        if (this.pattedFrame === 0) {
            ...
            //     gs * cat.draw_height);
            ctx.drawImage(this.cat.asset["img"], 0, 0, this.cat.sourceW, Math.max(this.cat.sourceH * this.level, 1), gs * -100, gs * (-this.cat.drawHeight + (this.cat.drawHeight * (1 - this.level))), gs * 200, gs * this.cat.drawHeight * this.level);
        }
